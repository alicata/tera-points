...
struct PointCloudLoader {
	...
	vector<shared_ptr<PointCloud>> files;
	...
	PointCloudLoader(shared_ptr<Renderer> renderer);
	void add(vector<string> files, std::function<void(vector<shared_ptr<PointCloud>>)> callback);
	void spawnLoader();
	void process();
};